#!/usr/bin/env python3
"""Run or resume a GPT-5.6 Luna Max worker through Codex CLI."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        default=os.environ.get("CODEX_MAESTRO_WORKER_MODEL", "gpt-5.6-luna"),
    )
    parser.add_argument(
        "--effort",
        default=os.environ.get("CODEX_MAESTRO_WORKER_EFFORT", "max"),
    )
    parser.add_argument(
        "--sandbox",
...
def main() -> int:
    ...
    if return_code == 0 and args.session_file and session_id:
        ...
        print("Warning: Codex completed but no worker session ID was found.", file=sys.stderr)
    return return_code
