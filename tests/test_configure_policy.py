...
class ConfigurePolicyTests(unittest.TestCase):
    ...
    def test_generated_policies_use_current_capability_routing(self) -> None:
        ...
        self.assertIn("Default to `gpt-6-astra` at medium effort", codex)
        self.assertIn("Do not spawn native or CLI workers in default mode", codex)
        self.assertIn("Use economy mode only when explicitly requested", codex)
        self.assertNotIn("Default to Balanced", codex)
