...
class ConfigurePolicyTests(unittest.TestCase):
    ...
    def test_generated_policies_use_current_capability_routing(self) -> None:
        ...
        self.assertIn("`gpt-6-astra` at medium effort", codex)
        self.assertIn("working alone", codex)
        self.assertIn("fail-closed routing preflight", codex)
        self.assertIn("Do not automatically spawn", codex)
        self.assertIn("not supported presets", codex)
